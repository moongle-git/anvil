...
**상태**: **부분 폐기 — 아래 "로컬 도구 전제이므로 배포 인프라는 고려하지 않는다" 한 문장이 ADR-018로 폐기됨 (2026-07-18).** 나머지(Next.js App Router · npm workspaces · `src/types` 단일 소스)는 **그대로 유효하다.** 원문은 지우지 않는다 — 그때 왜 그렇게 판단했는지가 기록이다.
...
### ADR-018: 배포는 단일 VM에 두고 접근 통제는 앱 바깥 엣지에 둔다 (ADR-006의 로컬 도구 전제를 폐기)
**결정**: 이 도구를 공인 HTTPS로 서비스한다. 실행 환경은 **단일 VM** 하나이고, 웹은 systemd 유닛(`anvil-web`)으로 뜨며, 접근 통제는 **앱이 아니라 리버스 프록시(Caddy)의 basic auth 한 겹**이다. 빌드는 서버에서 하고 배포는 사람이 `scripts/deploy.sh`를 실행하는 것이다. **앱 코드는 배포를 위해 한 줄도 바뀌지 않는다.**

**뒤집는 결정**: ADR-006의 *"로컬 도구 전제이므로 배포 인프라는 고려하지 않는다"* 를 폐기한다. **ADR-006의 나머지 — Next.js App Router · npm workspaces · `src/types` 단일 소스 — 는 전부 유효하다.** 폐기되는 것은 "배포를 고려하지 않는다"는 **범위 선언 한 문장**이지 웹 UI의 구조가 아니다. PRD의 비목표 목록에서도 **배포만** 덜어낸다 — 인증·멀티테넌시는 비목표로 남는다(아래 (b)).

절차는 이 ADR에 적지 않는다. **런북은 `docs/DEPLOY.md`가 단독으로 소유한다** — 여기에 커맨드를 복사하면 두 개의 진실이 된다.

---

**(a) 단일 VM은 선택이 아니라 이 아키텍처의 논리적 귀결이다.**

두 개의 기존 결정이 이것을 못박는다.

- **ADR-007**: 웹은 `POST /api/runs`를 받으면 **같은 머신에서** `npm run consult -- --resume <runId>`를 detached 자식으로 spawn한다.
- **ADR-014**: 실행 상태·산출물의 단일 진실 공급원은 **로컬 SQLite 파일** 하나이고, WAL 모드가 해결하는 것은 "쓰는 프로세스 1 + 읽는 프로세스 N"이되 **같은 파일시스템 위에서만** 그렇다.

즉 웹과 CLI는 같은 커널·같은 파일시스템을 공유해야 한다. 인스턴스가 둘이면 A가 만든 run을 B가 실행할 방법이 없고, 목록이 노드마다 갈린다. **수평 확장·서버리스·다중 컨테이너는 성능 판단이 아니라 구조적으로 불가능하다.** 단일 VM은 이 설계를 배포에 그대로 번역한 것이다.

**기각한 대안**:
- **Docker** — 웹과 CLI가 spawn 관계라 **한 컨테이너에 있어야 하고**, DB는 볼륨으로 밖에 나가야 한다. 그러면 컨테이너화가 주는 이점(프로세스 격리, 이미지 단위 롤백, 재현 가능한 런타임)의 대부분이 상쇄되는 반면 spawn 경로 가정(`ANVIL_REPO_ROOT`)과 `node:sqlite`의 파일 경로 가정은 **그대로 남는다.** 얻는 것 없이 층이 하나 는다.
- **PaaS(Vercel·Fly 등)** — 파일시스템이 휘발성이거나 요청 단위로 격리된다. 상태가 로컬 파일이고 작업이 최악 6분 도는 detached 자식이라(ADR-012, ADR-016) 전제가 정면으로 어긋난다.
- **CI/CD 파이프라인** — 배포 대상이 서버 하나이고 배포하는 사람도 하나다. 워크플로 정의·시크릿 저장소·러너를 유지하는 비용이 `./scripts/deploy.sh` 한 줄을 대체할 만큼의 값을 내지 않는다. **자동화가 아니라 순서 보장이 목적**이므로, 스크립트는 출력을 보면서 사람이 실행한다.

---

**(b) 접근 통제는 앱 바깥에 둔다 — 그리고 왜 로그인을 만들지 않았는가.**

막으려는 것은 이론적 무단 접근이 아니라 **청구서**다. 인증 없이 공인 IP에 올리면 누구나 `POST /api/runs`로 운영자의 Gemini 키를 태울 수 있다. run 하나의 비용 구조와 그 65%가 `context-hunter`라는 사실은 ADR-016이 실측했다.

앱에 로그인을 만들지 않은 이유는 네 가지다. **"나중에 하겠다"가 아니라, 이 위협에 대해 로그인이 더 나은 답이 아니기 때문이다.**

1. **로그인은 이 위협을 더 잘 막지 못한다.** basic auth는 **Caddy에서 끊긴다 — Next 프로세스가 실행조차 되지 않는다.** 앱 로그인은 반대로 로그인 엔드포인트가 **공개되어야 동작하므로** 공격 표면이 오히려 는다(그 엔드포인트는 인증 없이 도달 가능해야 하고, 세션 저장소·쿠키·CSRF가 따라온다). 회원가입까지 열면 익명 트래픽이 정식 사용자가 되어 더 나빠진다. **로그인은 인증(authentication)이지 인가(authorization)도 비용 통제도 아니다.**
2. **CLI에는 세션이 없다.** `npm run consult`는 헤드리스 프로세스이고, 애초에 웹보다 먼저 있던 진입점이다(ADR-001·ADR-004). 인증을 앱에 넣으면 그것은 **웹 경로에만 존재**하고, DB에는 세션 없는 프로세스가 만든 run이 계속 쌓인다. 단일 진실 공급원(ADR-014)이 **두 개의 접근 모델**을 갖게 된다 — 그것은 인증이 아니라 인증의 외관이다.
3. **DB에 사용자 개념이 없다.** 운영자 한 명 전제에서 `runs.user_id`는 **항상 같은 값이 들어가는 컬럼**이고, 스코프 검사는 **항상 참인 조건**이다. `runStore`의 모든 조회와 삭제·재실행 경로를 고쳐서 얻는 것이 없다.
4. **엣지는 앱을 모른다.** 그래서 이 결정이 앱 코드를 오염시키지 않는다. 접근 통제를 바꾸는 것은 `/etc/caddy/Caddyfile`을 고치고 reload하는 일이지 재배포가 아니다.

**한계를 숨기지 않는다. 알고 선택한 것이다.**

| 한계 | 내용 |
|---|---|
| 단일 공유 자격증명 | 사람이 여럿이면 같은 비밀번호를 나눠 쓴다 |
| 앱은 접속자를 모른다 | 누가 무엇을 했는지 앱도 DB도 구분하지 못한다. 엣지 접근 로그만 남는다 |
| 표준적인 로그아웃이 없다 | 브라우저가 자격증명을 캐시한다. 끊으려면 비밀번호를 바꾸고 reload하는 것이 사실상 유일한 방법이다 |
| base64는 암호화가 아니다 | 자격증명이 매 요청 헤더에 실려 간다. **TLS는 선택이 아니라 이 설정의 성립 조건이다** — HTTP로 내리는 순간 평문 비밀번호를 반복 전송하는 것과 같다 |
| rate limit이 없다 | Caddy 표준 배포판에 해당 디렉티브가 없고, 그것 때문에 커스텀 빌드를 하지는 않는다. 방어는 **길고 랜덤한 비밀번호**뿐이다 |

**번복 조건**: **여러 사람이 각자의 run 목록을 갖는 요구가 생기면 이 결정은 뒤집힌다.** 그 시점에는 위 논거 1·3이 통째로 무효가 된다 — 사용자가 여럿이면 `user_id`는 더 이상 항상 같은 값이 아니고, 스코프 검사도 더 이상 항상 참이 아니다.

다만 **그때도 로그인만으로는 부족하다.** 이 도구는 요청 하나가 운영자의 API 키로 실비를 태우고, 그 비용은 이미 관측 가능하다(ADR-016의 `usage` 테이블). **사용자별 비용 쿼터가 인증과 한 세트로 따라와야 한다** — 인증만 붙이면 "누가 태웠는지 아는 상태로 계속 태우는 것"이 되고, 그것은 지금보다 나은 상태가 아니다. 위 (b)2가 지적한 CLI의 세션 부재도 그때 함께 답해야 한다. **이것은 계획이 아니라 조건이다. 그 결정은 아직 내려지지 않았다.**

---

**(c) 진행 중인 run은 배포보다 오래 산다.**

systemd의 기본값 `KillMode=control-group`은 stop/restart 시 그 서비스의 cgroup에 남은 **모든 프로세스**에 SIGTERM을 보낸다. 그런데 `spawnConsult`의 `detached: true` + `unref()`는 **프로세스 그룹과 세션만 분리할 뿐 cgroup 소속은 바꾸지 못한다.** 기본값으로 두면 **배포할 때마다 실행 중이던 consult run이 조용히 죽는다.**

따라서 유닛에 **`KillMode=process`** 를 둔다 — 메인 프로세스(`next start`)에만 시그널을 보내고 자식은 살려둔다. 살아남은 CLI는 자기 일을 마치고 DB에 쓰며, `runs`·`steps`가 단일 진실 공급원이므로(ADR-014) 웹이 재시작해도 브라우저 폴링이 그대로 이어진다.

**치명적이지는 않다** — 죽어도 resume이 있다(ADR-004). 그러나 run 하나는 Gemini 실비를 태우며 최악 6분 도는 작업이고(ADR-012, ADR-016), **이미 청구된 호출을 이유 없이 버릴 이유가 없다.** ADR-016이 "형식이 실패한 것이지 청구가 실패한 게 아니다"라며 실패한 시도까지 장부에 남긴 것과 같은 감각이다.

`scripts/deploy.sh`의 `pgrep` 게이트는 **이것과 다른 사고를 막는다** — 재시작이 아니라 **빌드**가 실행 중인 run의 소스를 갈아엎는 것(tsx가 소스를 실행 중에 읽는다). 두 겹이 각각 다른 것을 막으므로 하나로 합칠 수 없다.

---

**(d) 빌드는 서버에서 한다 — 그리고 devDependencies는 런타임 의존이기도 하다.**

`npm run consult`는 **`tsx`로 TypeScript 소스를 직접 실행한다.** 그런데 `tsx`는 devDependency다. 즉 이 프로젝트에서 **devDeps는 개발 도구가 아니라 CLI 파이프라인의 런타임 의존이다.**

`npm ci --omit=dev`(또는 셸에 `NODE_ENV=production`이 새어 들어와 npm이 devDeps를 건너뛴 경우)로 설치하면 **웹은 멀쩡히 뜨고 run만 죽는다.** 그리고 `spawnConsult`가 `stdio: "ignore"`로 자식을 띄우므로(ADR-007) **에러가 어디에도 남지 않는다** — 로그는 조용하고, 웹은 runId를 정상 응답하며, run은 영원히 `pending`에 머문다. 원인이 보이지 않는 실패다.

**서버 빌드는 이 문제를 구조적으로 없앤다.** 빌드하려면 devDeps가 있어야 하므로, 빌드하는 머신이 곧 실행하는 머신이면 **런타임에 devDeps가 없는 상태 자체가 성립하지 않는다.** 그래서 `scripts/deploy.sh`는 `--include=dev`를 **명시**하고(환경변수가 새어 들어와도 무력화된다), 이 사실은 CLAUDE.md에도 CRITICAL로 올라간다 — 문서 한 줄로 몇 시간을 아낄 수 있는 함정이다.

**기각한 대안**: **아티팩트를 로컬에서 빌드해 서버로 올리기** — 개발 머신은 arm64 mac이고 서버는 arm64 리눅스다. `next build`의 산출물과 `node_modules`가 플랫폼에 묶이므로 결국 서버에서 `npm ci`를 다시 돌게 되고, 그러면 서버 빌드와 달라지는 것이 없다.

---

**트레이드오프**:
- **가용성 보장이 없다.** 롤백·헬스체크 폴링·무중단 배포가 없고 `systemctl restart`의 다운타임 몇 초를 그대로 받는다. 운영자 1명의 내부 도구에 필요한 것이 아니다.
- **VM이 죽으면 데이터가 죽는다.** 백업은 자동화되지 않았다(`sqlite3 .backup` / `VACUUM INTO`를 사람이 돌린다 — `docs/DEPLOY.md` §4). 산출물은 재생성 가능하지만 **비용을 다시 낸다.**
- **비밀번호를 잃으면 되돌릴 수 없다.** 해시만 서버에 있다. 평문은 비밀번호 관리자가 소유한다.
- **엣지 설정이 레포 밖에 산다.** `/etc/caddy/Caddyfile`은 서버에만 존재하고 레포에는 플레이스홀더 예시(`deploy/Caddyfile.example`)만 있다. 실제 값(호스트·해시)은 커밋되지 않으므로 **서버에서 직접 고친 것이 레포에 반영되지 않는다.** 값이 아니라 구조가 바뀌면 예시 파일도 함께 고쳐야 한다.

**하위호환**: **로컬 실행 경로는 하나도 바뀌지 않는다.** `npm run consult`·`npm run web`·`ANVIL_DB_PATH` 기본값 전부 그대로다. 이 phase가 추가한 것은 `deploy/` 4개 파일 · `scripts/deploy.sh` · `docs/DEPLOY.md`이고, 앱 소스(`src/`·`web/src/`)는 **한 줄도 바뀌지 않았다.** 배포하지 않는 사람에게 이 phase는 존재하지 않는 것과 같다.

### ADR-019: 주제를 사용자 입력에서 자본 흐름 탐색으로 확장하고, 귀속을 스키마로 강제한다
**결정**: 파이프라인의 입력을 "사용자가 적어 온 아이디어" 하나에서 **두 모드**로 넓힌다. 기존 모드(직접 입력)는 그대로 두고, 새 모드(**주제 찾기**)는 자본 신호를 검색해 서비스 기회 후보를 3~5개 제시하고 사람이 그중 하나를 고른다. 고른 후보가 `runs.idea`로 확정되면 **그 뒤 5단계 서사는 한 글자도 바뀌지 않는다.**

**뒤집는 결정**: 없다. **이것은 확장이지 폐기가 아니다.** ADR-004(하네스 패턴)·ADR-007(spawn + 폴링)·ADR-013(출처는 사실이다)·ADR-014(SQLite)·ADR-016(usage)·ADR-017(스키마 팩토리)은 전부 **그대로 유효하고, 이 phase는 그것들을 재사용해서 만들어졌다.** 새로 만든 인프라가 없다는 것이 이 결정의 요지 중 하나다(결정 1).

---

**(1) 두 모드는 공존하고, 스카우트는 별도 개체가 아니라 파이프라인의 앞 step이다.**

`trend-scout`을 `PIPELINE_STEPS`의 **맨 앞**에 넣는다. 별도 `scouts` 테이블도, 별도 라우트 트리도 만들지 않는다.

**별도 개체로 분리하지 않은 결정적 이유는 비용 장부다.** `usage.run_id`는 `NOT NULL` FK다(ADR-016). run이 아닌 개체가 Gemini를 호출하면 **그 호출의 비용을 적을 곳이 없다** — ADR-016이 세운 "모든 Gemini 호출은 재시도와 실패한 시도까지 포함해 usage 테이블에 기록된다"가 통째로 깨진다. 그 규칙을 지키려고 `usage`에 nullable `scout_id`를 더하는 것은, 관측 테이블에 두 번째 외래키를 만들어 모든 집계 질의를 분기시키는 일이다.

앞 step으로 두면 반대로 **전부 공짜로 따라온다**: `usage`·`steps`·`artifacts`가 그대로 쓰이고, 삭제(CASCADE)·재실행(포크)·비교(`/compare`)·resume이 코드 한 줄 없이 성립한다. 스카우트 run도 그냥 run이기 때문이다.

**pause/resume은 `interviewer`가 이미 만들어 둔 선례를 그대로 쓴다.** 웹이 spawn하는 CLI는 detached이고 `stdio: "ignore"`라(ADR-007) **stdin이 없다** — 프로세스 안에서 사람에게 물어볼 방법이 구조적으로 없다. 그래서 `interviewer`가 `questions` 아티팩트를 남기고 `status: "waiting"`으로 멈췄다가 `answers`가 도착하면 재개하듯, `trend-scout`은 `opportunities`를 남기고 멈췄다가 `selection`이 도착하면 재개한다. 두 단계는 **상호배타적이다** — `createRun`이 스카우트 run에 `interviewer`를 seed하지 않고, orchestrator의 인터뷰 블록 조건도 `state.interview && !state.scout`이다. 한 run에서 사람을 두 번(후보 선택 → 질문 답변) 멈춰 세우지 않는다.

**대가를 정확히 적는다**:
- **`runs.idea`가 가변이 된다.** 스카우트 run의 초기 `idea`는 주제가 아니라 사용자가 준 **범위 힌트**이고(없으면 `"전 범위 탐색"`), 후보가 선택되면 `title — whatItIs`로 갈아끼워진다. 새 메서드를 만들지 않는다 — `saveRun`이 이미 `idea`를 UPDATE하고, 같은 선택으로 여러 번 돌아도 같은 값이 되므로 멱등하다.
- **`run_id` slug에 초기 힌트가 남는다.** run id가 `…-전-범위-탐색-6cfb90` 꼴이라 확정된 주제와 어긋난다. 불투명 식별자이므로 동작에 영향이 없고, id를 나중에 고치는 것은 FK 3개를 끌고 다니는 일이라 하지 않는다. **어긋남을 알고 남긴다.**

---

**(2) `scout`은 컬럼이 아니라 `steps`에서 파생한다.**

`state.scout`은 `runs` 테이블의 컬럼이 아니라 **`steps`에 `trend-scout` 행이 있는가**로 계산된다.

**컬럼을 더하지 않은 이유는 취향이 아니라 함정이다.** `src/lib/db.ts`의 DDL은 전부 `CREATE TABLE IF NOT EXISTS`다. 즉 **테이블이 이미 있으면 그 안의 컬럼 정의는 아무 효과도 내지 않는다** — 기존 `data/anvil.db`에 새 컬럼이 생기지 않고, 코드는 있다고 믿는다. `usage` 추가(ADR-016)가 무사했던 것은 그것이 **통째로 새 테이블**이라 `IF NOT EXISTS`가 정확히 의도대로 동작했기 때문이다. 그리고 ADR-014가 **마이그레이션 러너를 명시적으로 금지했으므로**(*"이 프로젝트에 마이그레이션 러너는 존재하지 않으며, 만들지 마라"*) 러너를 만드는 것도 답이 아니다.

파생으로 두면: **DDL 변경 0 · `SCHEMA_VERSION` 그대로 2 · 마이그레이션 0 · 기존 DB 파일이 그대로 열린다.**

**그런데 `interview`는 왜 컬럼으로 남는가.** 두 값이 다른 것을 뜻하기 때문이다. `interview`는 "이 run이 인터뷰를 **쓰기로 했는가**"라는 **의도**이고, `interviewer` step의 존재는 그 의도의 **결과**다. 둘은 갈릴 수 있다 — 인터뷰가 켜졌는데 모델이 물을 것이 없다고 판단해 질문이 0개면, 의도는 참인데 step은 즉시 `completed`가 된다. 의도를 결과에서 역산하면 그 run은 "인터뷰를 안 쓴 run"으로 잘못 읽힌다. `scout`에는 그런 간극이 없다 — 스카우트 run에는 **반드시** `trend-scout` step이 있고, 아닌 run에는 **결코** 없다. 파생이 안전한 것은 이 동치 때문이지 파생이 일반적으로 낫기 때문이 아니다.

---

**(3) grounded 호출을 검색과 합성으로 쪼갠다.**

한 번의 grounded 호출로 "검색해서 기회 후보를 내라"고 하지 않는다. **두 호출로 나눈다** — `scout-search`(grounded, 산출물은 **평평한 사실 목록**)와 `trend-scout`(non-grounded, 산출물은 후보). 앞에 `scout-planner`(non-grounded, `thinkingBudget: 0`)가 축별 검색어를 만든다. 세 호출은 각자의 usage 라벨로 장부에 남는다.

**세 가지 이유가 전부 독립적으로 이 분리를 요구한다.**

1. **인용을 ID로 지목시키려면 두 호출이어야 한다.** grounding 인용의 uri는 `vertexaisearch.cloud.google.com/grounding-api-redirect/...` 형태의 **만료되는 리다이렉트**다(ADR-012에서 확인). 이 URL은 LLM에게 보여줘도 판단에 아무 정보를 주지 않고, 받아적게 하면 `cloud.google.google.com`이 나온다(ADR-013의 스모킹건). 그래서 **코드가 인용에 `C1`·`C2` 번호를 붙여 다음 호출의 프롬프트에 넣고, 모델은 ID로만 지목한다** — `contextHunter`가 커뮤니티 목소리를 `V1`·`V2`로 넘기는 것과 **같은 패턴이다**. 한 호출 안에서는 이것이 불가능하다: 인용은 그 호출의 **응답에** 실려 오므로, 같은 호출의 프롬프트에 넣을 수 없다.
2. **grounding은 `responseJsonSchema`를 쓸 수 없다.** 자유 텍스트 → `extractJsonText` → zod 경로가 유일한데(ADR-013), 후보 스키마는 `candidates[].signals[].figures[]`까지 3중 중첩이다. 그 깊이를 자유 텍스트로 받으면 형식 실패가 쏟아진다. 합성을 non-grounded로 두면 구조화 출력이 형식을 보장한다.
3. **그래서 비싸진다.** ADR-016의 실측이 `context-hunter` 하나가 run 비용의 65%를 먹은 원인으로 지목한 것이 정확히 **"grounding 정액 요금 + 형식 실패 재시도"** 조합이다. 형식 실패한 재시도는 프롬프트 전문을 다시 보내면서 `$0.035` 정액을 **다시** 태운다. 중첩 깊은 스키마를 grounding에 물리는 것은 그 복권을 매 run 돌리는 것과 같다.

`scout-search`는 `useUrlContext: false`다. `contextHunter`가 그것을 켜는 것은 **이미 이름이 나온** 경쟁사의 공식 페이지를 읽기 위해서인데, 여기서는 읽을 대상 URL이 사전에 없어 왕복 지연만 늘어난다.

---

**(4) 환각 방어는 프롬프트가 아니라 스키마다 — 그리고 그 경계를 정확히 긋는다.**

이 파이프라인에서 환각 위험이 가장 높은 자리가 여기다. 모델은 훈련 데이터의 과거 트렌드를 현재로 착각해 뱉고, 그럴듯한 금액과 URL을 지어낸다. **부탁으로는 막을 수 없다** — ADR-017이 "능력이 없는 게 아니라 칸이 없다"며 부탁을 칸으로 바꾼 것과 같은 판단이다. 코드가 검사할 수 있는 것은 전부 `opportunitiesSchemaFor(constraints)`의 제약으로 만들고, 위반은 `generateStructured` **안에서** 자가 교정 재시도를 탄다(ADR-017 — 검증을 바깥으로 빼면 반환한 **뒤**라 재시도가 붙지 않고, 에러 메시지가 곧 교정 프롬프트다).

코드가 강제하는 것:

| 제약 | 내용 |
|---|---|
| **인용 화이트리스트** | 모든 `citationRef`가 **코드가 실제 grounding 응답에서 추출한** 인용 ID여야 한다. 목록에 없는 ID는 검증 실패다 |
| **삼각측량** | 후보마다 서로 다른 `signalType` **2종 이상** ∧ 서로 다른 `citationRef` **2개 이상**. 종류만 검사하면 같은 기사를 두 타입으로 라벨링해 우회한다 — **둘 다** 요구하는 이유다. `counterSignal`은 반대 증거이지 근거가 아니므로 이 계산에서 뺀다 |
| **`observedAt` 날짜창** | 모든 신호에 필수이고, 미래도 안 되고 탐색 구간(`SCOUT_LOOKBACK_MONTHS` = 18개월) 밖도 안 된다. 형식 검사가 아니라 **검색 없이는 채울 수 없는 필드를 만드는 것**이 목적이다 — 모델의 사전지식에는 날짜가 붙어 있지 않다. (`effectiveAt`은 미래여도 통과한다. 시행 예정 규제가 가장 가치 있는 신호다) |
| **수치 귀속** | `statement`의 금액·퍼센트 표기는 전부 `figures[]`에 같은 표기로 인용과 함께 담겨야 한다. 정규식을 **좁게** 잡는다 — 오탐은 모델이 만족시킬 수 없는 요구가 되어 **무한 재시도**를 만든다. 누락은 그 수치의 귀속을 강제하지 못할 뿐이라 훨씬 싸다 |
| **`counterSignal` 필수** | optional이 아니라 필수 필드다. 반대 증거를 못 찾는 주제는 검색되지 않은 주제다 |
| **침묵 허용** | `candidates`에 `min(1)`이 **없다.** 그리고 grounding 인용이 0건이면 합성 호출 **자체를 건너뛰고** 빈 배열로 끝낸다(침묵 게이트) |

**침묵 허용이 이 목록에서 유일하게 종류가 다르다.** 나머지가 전부 "지어내면 걸린다"인 반면, 이것은 **"지어낼 이유를 없앤다"**이다. 빈손으로 돌아올 길이 없는 시스템에서 모델은 반드시 무언가를 내놓게 되고, 그때 나오는 것이 환각이다. 인용이 0건인데 후보가 나왔다면 그 후보의 근거는 100% 사전지식이다. **침묵할 수 있어야 지어내지 않는다.**

해소 단계(`citationRef` → 실제 `Citation`)에도 같은 규율이 걸린다: 모르는 ref는 "가장 비슷한 인용"으로 **퍼지 매칭하지 않고 드롭한다**(퍼지 매칭은 환각을 그럴듯한 근거로 세탁한다 — `resolveVoiceRefs`와 같은 규율). 드롭 결과 신호가 2건 미만이 되거나 `counterSignal`이 드롭되면 **후보를 통째로 버린다** — 살려두면 스키마가 막으려던 것이 해소 단계라는 뒷문으로 들어온다.

**여기서 경계를 긋는다. ADR-013이 그은 것과 같은 선이다.**

> - **코드가 보장하는 것**: 후보의 모든 주장이 **실제로 검색된 문서를 가리킨다.** 참조 무결성·귀속·구조·날짜창.
> - **코드가 보장하지 못하는 것**: **그 문서가 정말 그 말을 하는가.**
>
> **이 구멍은 닫히지 않았다.** 모델은 실재하는 기사 [C3]을 지목하면서 그 기사에 없는 금액을 쓸 수 있고, 코드는 그것을 잡지 못한다. **"환각을 없앴다"고 말하지 않는다** — 없앤 것은 *출처 없는 주장*이지 *틀린 주장*이 아니다.

**원문 대조가 가능한지 조사했고, 불가능하다는 결론이 나왔다** (step 2, `@google/genai` 타입 전수 확인):

1. `groundingSupports[].segment.text`는 **출처가 아니라 모델 자신의 응답 텍스트**다(타입 주석이 "from the response"이고 offset도 응답 Part 기준이다). 그것으로 대조하면 **모델의 주장을 모델의 주장과 비교하는 순환**이 된다.
2. `groundingChunk.web`에는 `domain`·`title`·`uri`뿐이고 **본문이 없다.**
3. 본문을 담는 `groundingChunk.retrievedContext.text`는 **Vertex AI Search/RAG 전용**이라 `googleSearch` 경로에서는 채워지지 않는다.

따라서 **세그먼트 추출기를 만들지 않았고 `extractCitations`도 손대지 않았다** — 조사 결론만 `src/services/gemini.ts`에 주석으로 남겼다. `CapitalSignal.quote`는 optional로 두고 **코드로 대조하지 않는다.** 대신 후보 선택 화면(웹)과 리포트 머리말에 그대로 실어 **사람의 눈**에 맡긴다. 귀속의 강제는 `quote`가 아니라 화이트리스트가 담당한다.

**얻는 것은 자본 흐름의 공개된 그림자다.** 자본 데이터의 1차 사료(PitchBook·CB Insights·산업 리포트)는 **유료 장벽 뒤에 있어 Google Search grounding이 닿지 못한다.** 잡히는 것은 그것을 인용한 기사·보도자료·블로그다. 네 축 중 **`regulation`만이 1차 사료가 완전히 공개된 축이다** — 관보·법령·시행령은 무료이고 검색 가능하며, **시행일이 박힌 규제는 기업에 강제 지출을 만들고 그 시행일은 아직 오지 않은 미래라 사전지식으로는 알 수 없다.** 그래서 플래너 프롬프트가 네 축 중 `regulation`에 가장 큰 비중을 싣는다. 이것은 선호가 아니라 **데이터 접근성의 귀결**이다.

---

**(5) 스카우트는 점수를 매기지 않고, 검증하지 않는다.**

후보 스키마에 **점수·순위·랭킹·추천 필드가 없다.** 웹 UI도 모델이 낸 순서를 그대로 렌더하고 뱃지를 붙이지 않는다.

**근거는 ADR-010이 판정자를 분리한 근거와 같다.** ADR-010은 合을 설계한 에이전트가 스스로 채점하면 낙관 편향이 들어간다며 판정을 제3의 에이전트로 뺐다. 주제를 **발굴한** 에이전트가 그 주제에 점수를 매기는 것은 정확히 같은 형태의 자기채점이다. 게다가 파이프라인 **완주 전에 결론이 나오면** 뒤의 正·反·合·판정이 할 일이 없어지고, ADR-008이 금지한 "결론 선노출"이 리포트가 아니라 **선택 화면에서** 일어난다.

**"이미 레드오션인가"도 앞단에서 거르지 않는다.** 그것은 `cold-critic`(反)의 일이다 — 카피캣 리스크는 反의 3축 중 하나로 이미 명시돼 있다. 앞에서 미리 거르면 **反이 공격할 표적이 사라진다.** 表적을 세우는 것이 正의 역할이고 그것을 부수는 것이 反의 역할인데, 스카우트가 부술 만한 것을 미리 치워버리면 변증법이 성립하지 않는다.

**`counterSignal`은 이 규율의 예외처럼 보이지만 아니다.** 그것은 후보를 **걸러내지 않는다** — 반대 증거를 찾아 **주석으로 달 뿐**이고, 불리한 증거가 있다고 해서 후보가 탈락하지 않는다. **판정이 아니라 사실 요구다.** 그리고 그 사실은 하류로 그대로 흐른다: `contextHunter` 프롬프트의 스카우트 섹션에 "반대 증거" 소제목으로 **반드시** 실리고, 리포트 머리말과 선택 화면에도 접히지 않은 채 노출된다. 빼면 시장 맥락부터 낙관으로 기울고, 그 기울기가 正·反·合 **전부에** 주입된다.

같은 이유로 스카우트의 인용은 **`context.citations`에 병합하지 않는다.** 그 필드는 "이 grounding 호출이 실제로 검색해 코드가 추출한 인용"이라는 단일 의미를 갖는다(ADR-013). 스카우트 인용은 `opportunities` 아티팩트에 온전히 남아 리포트와 웹이 **별도 섹션**으로 표시한다. 그리고 `contextHunter`의 스카우트 섹션은 신호를 "검증된 사실"이 아니라 **교차 확인 대상**으로 제시한다 — 그 호출이 앞 단계의 주장을 승계하지 않고 독립 근거를 만들게 하기 위해서다.

---

**기각한 대안**:
- **별도 `scouts` 테이블 + 별도 라우트** — 위 결정 1. `usage.run_id`가 NOT NULL FK라 비용 장부가 깨진다. 그리고 삭제·재실행·비교·resume을 전부 다시 만들어야 한다.
- **한 번의 grounded 호출로 검색과 후보 생성을 동시에** — 위 결정 3의 세 가지 이유. 특히 인용에 번호를 붙여 되먹일 자리가 사라져 **ID 지목 자체가 불가능해진다.**
- **`runs`에 `scout` 컬럼 추가** — 위 결정 2. DDL이 `IF NOT EXISTS`뿐이라 기존 DB에 반영되지 않고, 코드는 있다고 믿는다. 조용한 실패다.
- **후보에 유망도 점수를 매겨 정렬** — 위 결정 5. ADR-010 위반이고 ADR-008의 결론 선노출을 선택 화면으로 옮긴다.
- **인기도 기반 소스(HN 인기글·YouTube 급상승·네이버 데이터랩)로 주제 발굴** — 이미 `research/`에 세 소스의 수집기가 있어 **가장 싸게 만들 수 있는 길이었다.** 그러나 조회수·화제성은 **자본 흐름과 다른 축이고, 대개 이미 늦은 신호다** — 기사가 트렌드를 이야기할 즈음이면 그 시장에는 경쟁자가 있다. 이 phase가 겨냥하는 것은 인기가 아니라 **자본이 들어간 시점과 물건이 나오는 시점 사이의 시차**다. 싸다는 이유로 다른 것을 만들지 않는다.
- **`quote`를 `groundingSupports`로 코드 대조** — 위 결정 4. 조사 결과 **얻을 수 있는 것이 출처 원문이 아니라 모델 자신의 응답 텍스트**라 대조가 순환이 된다. 하지 않았고, 하지 않았다고 적는다.
- **CLI에 스카우트 모드 추가** — `interviewer`가 CLI에 없는 것과 같은 이유다. spawn된 CLI는 detached라 stdin이 없어 후보 선택을 받을 수단이 없다. 스카우트 run은 웹에서만 만들어진다.

**번복 조건**:
- **`citations`가 지속적으로 0건이면** 침묵 게이트가 매번 걸려 이 모드는 빈 후보만 내놓는 기능이 된다. 그때 뒤집히는 것은 이 ADR이 아니라 **소스 선택**이다 — grounding이 자본 신호에 닿지 못한다는 뜻이므로, 유료 API(결정 4에서 유료 장벽으로 기각한 그 소스들)를 도입할지가 비용 문제로 다시 열린다.
- **후보의 주장이 지목한 문서와 어긋나는 사례가 관측되면**(결정 4가 열어 둔 구멍이 실제로 새면) `quote`를 사람의 눈에 맡기는 현재 처리가 부족하다는 뜻이다. 그때의 답은 프롬프트 강화가 아니라 **본문을 실제로 가져오는 경로**(urlContext로 원문을 읽어 대조)이고, 그것은 왕복 비용과 함께 별도로 결정해야 한다.
- **한 run에서 여러 후보를 동시에 돌리는 요구가 생기면** "한 run = 한 주제"라는 이 phase의 전제가 무효가 된다. 그때는 `runs.idea` 단수 전제와 `/compare`의 2-run 비교가 함께 흔들리므로, 결정 1(앞 step으로 둔다)부터 다시 검토해야 한다.

---

#### 실측 결과 (2026-07-19, phase 10 step 9) — 계측이 **해피패스가 막혀 있음**을 잡아냈다

ADR-016·ADR-017의 선례를 따라 **추정이 아니라 실측을 근거로 삼는다.** 설계 단계에서 "스카우트 모드 run은 대략 1.5배"라고 추정했으나, 그 추정은 **검증할 수 없었다** — 아래 이유로 스카우트 파이프라인이 후보 생성까지 도달하지 못했기 때문이다.

측정: `gemini-2.5-flash`·스크래치 DB(`ANVIL_DB_PATH`)·웹 `POST /api/runs {mode:"scout"}`로 **n=3** run. 범위 힌트를 셋 다 다르게 줬다(없음 / `"기업용 AI 인프라"` / `"EU AI Act 규제 대응"`).

**1. 세 run 모두 후보 0개로 끝났다 — 그러나 원인이 둘로 갈린다.**

| run | 범위 | 도달 지점 | 원인 |
|---|---|---|---|
| `6cfb90` | (없음, 전 범위 탐색) | `scout-search`까지 | **인용 0건 → 침묵 게이트** |
| `d94901` | 기업용 AI 인프라 | `scout-search`까지 | **인용 0건 → 침묵 게이트** |
| `e12980` | EU AI Act 규제 대응 | **합성 호출까지 도달** | **Gemini API가 스키마를 거부(400)** |

**2/3은 설계된 동작이다.** 인용이 0건이라 합성을 건너뛰고 `candidates: []`로 끝냈고, orchestrator가 `SCOUT_NO_CANDIDATES_MESSAGE`로 `error`를 기록했다. 침묵 게이트가 **의도대로 작동한다는 최초의 야생 관측이다.** (`citations` 0건 자체는 스카우트 고유의 문제가 아니다 — ADR-017의 phase 8 실측도 `context-hunter`에서 `citations` 0건을 기록했고 원인이 측정되지 않은 채 남아 있다. 같은 증상이 같은 자리에서 다시 나온 것이다.)

**나머지 1/3이 이 계측의 진짜 산출물이다.**

**2. `opportunitiesSchemaFor`가 만드는 JSON 스키마를 Gemini가 거부한다 — 결정적이고 재현 가능하다.**

```
400 INVALID_ARGUMENT
The specified schema produces a constraint that has too many states for serving.
Typical causes ... schemas with long array length limits (especially when nested) ...
```

**이것은 확률적 형식 실패가 아니라 결정적 거부다.** `superRefine`은 `z.toJSONSchema`에 나타나지 않으므로 API로 가는 JSON 스키마는 **매 호출 동일하다**(1,748바이트). 직접 호출로 확인했고, 원인을 이분 탐색으로 좁혔다:

| 변형 | 결과 |
|---|---|
| A. 현행 draft 스키마 그대로 | **거부** (too many states) |
| B. 최상위 문자열의 `minLength(1)` 제거 | **거부** |
| C. **배열 경계(`candidates.max(5)`·`signals.min(2)`) 제거** | **통과** |
| D. 후보 객체 1개(배열 없음) | **통과** |

원인은 **중첩된 배열 길이 제한**이다 — `candidates[≤5] → signals[≥2] → figures[]`의 3중 중첩에 경계가 걸리면서 Google의 제약 엔진이 상태 폭발을 일으킨다. 에러 메시지가 지목한 바로 그 항목이다.

**결과: 이 phase의 해피패스는 한 번도 성립한 적이 없다.** 인용이 0건이면 침묵 게이트가 먼저 막고, 인용이 있으면 합성 호출이 400으로 죽는다. `trend-scout` usage 라벨은 **3/3 run에서 행이 하나도 생기지 않았다** — 과금 이전에 거부되기 때문이다. 후보가 실제로 렌더된 것을 **아무도 본 적이 없다**(step 3~8의 테스트는 전부 Gemini mock이라 이 제약을 만나지 않는다. mock이 잡지 못하는 종류의 실패다).

> **고치지 않고 기록만 한다.** step 9의 scope는 문서이고, 이 phase의 규칙은 *"구현이 틀렸다면 고치지 말고 ADR에 기록한 뒤 별도 step으로 남겨라"*이다. **다음 phase의 1순위 작업이다.** 방향은 실측이 이미 가리킨다 — 배열 경계를 JSON 스키마에서 빼고 **`superRefine`에서 검사하면 된다**(위 변형 C). 그러면 위반이 여전히 자가 교정 재시도를 타므로(ADR-017) 강제력은 유지되고, API로 가는 스키마만 단순해진다. **다만 이것도 추정이다 — 고친 뒤 실제로 후보가 나오는 것을 보기 전까지는 확인된 것이 아니다.**

**3. 비용 — 측정된 것만 적는다.**

| label | 콜 | 입력 | 출력 | thinking | USD (3 run 합) |
|---|---|---|---|---|---|
| `scout-planner` | 3 | 2,620 | 540 | 0 | $0.00214 |
| `scout-search` | 3 | 2,385 | 10,769 | 10,016 | **$0.15768** |
| `trend-scout` | **0** | — | — | — | **— (호출이 성립하지 않았다)** |
| **합계** | 6 | 5,005 | 11,309 | 10,016 | **$0.15981** |

run당 평균 **$0.0533**이고 그중 **$0.0526(98.7%)이 `scout-search` 하나**다. `scout-planner`는 `thinkingBudget: 0`이라 run당 $0.0007로 사실상 무료다 — 세 번째 호출을 만들면서 비용이 늘까 걱정한 것은 기우였다.

`scout-search`가 지배하는 구조는 **ADR-016이 `context-hunter`에서 관측한 것과 동일하다**: grounded 정액 요금 $0.035가 콜당 고정으로 붙고, 거기에 thinking(run당 3,339토큰 평균)이 얹힌다. 즉 **grounded 호출 1회 = $0.04~0.06**이 이 코드베이스의 반복되는 단가다.

**추정과의 대조**: 참고로 사용자 DB의 완주 run 3건은 $0.1427~$0.1543이다. 스카우트 앞단이 run당 $0.053을 더한다면 합계는 대략 $0.20 안팎, 즉 **1.35~1.4배**가 된다 — 설계 초안의 "1.5배" 추정과 방향은 맞다. **그러나 이것을 실측이라고 부르지 마라.** 합성 호출(`trend-scout`, budget 8192)이 **한 번도 성공하지 않아 그 비용이 장부에 없고**, 스카우트 run의 하류(`context-hunter` 이후)는 아예 돌지 않았다. 위 곱셈은 **측정된 앞단 + 다른 아이디어의 완주 run**을 이어 붙인 것이지 한 run의 실측이 아니다.

**4. 측정의 한계.**
- **n=3이고, 그중 후보 생성에 도달한 것은 1건이며, 그 1건은 실패했다.** 이 숫자로 분포를 말하지 마라.
- **`scout-search`의 비용 편차가 크다**($0.0401~$0.0640). 출력이 1,261~6,731토큰으로 5배 넘게 흔들리는데 run이 3개뿐이라 원인(범위 힌트의 폭? 검색 결과량?)을 분리할 수 없다. **소수점 두 자리를 신뢰하지 마라.**
- **`cost_usd`는 grounding 1,500건/일 무료 한도를 모델링하지 않는다**(ADR-016). 한도 안이면 `scout-search`의 실제 청구는 훨씬 작고, 그러면 위 표의 98.7%라는 비중도 달라진다.
- **후보 품질·유용성은 측정하지 않았다.** 후보가 하나도 만들어지지 않았으므로 측정할 대상이 없었다.

---

### ADR-020: 인용이 필요한 grounding 호출은 산문으로 받는다 (JSON 강제는 귀속을 지운다)

**결정**: Google Search grounding 호출에서 **코드가 추출한 인용(`groundingChunks`)이 필요하면, 그 호출에 JSON 출력을 강제하지 않는다.** 산문으로 받아 인용을 지키고, 구조화는 **뒤따르는 non-grounded 호출**이 한다. `scout-search`가 이 경로로 옮겨간다(`generateGroundedText`).

**뒤집는 결정**: ADR-019 결정 3의 **전제 하나**를 폐기한다. 그 결정은 grounded 호출이 "평평한 사실 목록"을 **JSON으로** 낼 수 있다고 전제했다 — 후보 스키마가 3중 중첩이라 형식 실패가 쏟아지는 것이 문제이지, 얕은 스키마면 괜찮다고 보았다. **그 전제가 틀렸다.** 깊이의 문제가 아니라 **JSON을 요구하는 것 자체**가 문제였다. 분리의 나머지(검색과 합성을 쪼갠다, 인용은 ID로 지목한다, 합성은 non-grounded다)는 **전부 그대로 유효하고, 이 결정은 같은 규율을 한 단계 더 내린 것이다** — 이제 검색 호출 안에서도 "검색"과 "구조화"가 갈린다.

---

**(1) 실측: 출력 형식과 인용 귀속은 맞바꿈 관계다.**

`groundingMetadata`는 **항상 온다.** 그런데 JSON 출력을 지시하면 그 안의 `groundingChunks`·`groundingSupports`가 **통째로 빈 채로** 온다. `webSearchQueries`는 정상이다 — **검색은 실재했고 귀속만 안 붙은 것이다.** 그래서 증상이 "검색 실패"가 아니라 "근거를 못 찾음"으로 나타난다.

같은 프롬프트에서 출력 형식만 갈라 실제 API를 8회 호출했다:

| 출력 형식 | `groundingChunks` 관측값 | 성공 |
|---|---|---|
| 산문 | 11, 13, 16, 6, 5 | **5/5** |
| JSON 강제 | 0, 0, 0, 0, 0, 0, 11 | **1/7** |

JSON 지시가 붙으면 모델이 검색을 **더 넓게** 돌리고(실측 2건 → 8건) 그 결과를 종합해버리는데, 귀속은 그 과정에서 떨어져 나간다. 메커니즘은 Google 내부이고 응답에 단서가 없다 — **관측된 상관을 그대로 적는다.**

**이것은 프롬프트 문구로 고칠 수 없다.** 두 번 헛짚었고 그 기록을 남긴다:
- *"검색어가 너무 많아서다"* → 축당 검색어에 상한을 걸었으나 **모델이 알아서 확장했다**(상한 8 → 실제 검색 11건). 상한은 남겼지만(`SCOUT_SEARCH_MAX_QUERIES_PER_AXIS`) 이것이 원인은 아니다.
- *"비결정적일 뿐이다"* → 같은 요청 재전송을 붙였다(`citationRetries`). 3연속 0건이 나와 부족함이 드러났다. 보험으로 남기되 **이것이 해결책이 아니다.**

---

**(2) 그래서 검색 호출도 둘로 쪼갠다.**

`searchCapitalSignals`가 두 호출이 된다:

1. `scout-search` (**grounded, 산문**) — 인용을 확보한다. 스키마도 JSON 지시도 없다.
2. `scout-structure` (**non-grounded, `thinkingBudget: 0`**) — 그 산문을 `ScoutDossier`로 **옮겨 적는다.** 새 사실을 만들지 못하게 프롬프트가 필경사 역할로 못박는다.

**구조화가 non-grounded인 것이 핵심이다.** 형식 실패에 ADR-004의 자가 교정 재시도가 그대로 붙고, 그 재시도가 **grounding 정액 요금($0.035)을 다시 태우지 않는다.** ADR-016이 `context-hunter`에서 지목한 "정액 + 형식 실패 재시도" 조합을 여기서 구조적으로 끊는다.

**인용 0건이면 구조화를 건너뛴다.** 침묵 게이트(ADR-019)가 어차피 전부 버릴 findings를 만드느라 토큰을 쓰지 않는다.
...
|---|---|---|
| 인용 | **0건** (3 run 연속) | **23건** |
| 관측(findings) | 35건 → 전량 폐기 | 26건 |
| 후보 | **0건** | **2건** (출처·날짜·반대 증거 포함) |
| 호출 | planner, search×3(재검색) | planner, search, structure, trend-scout×2 |

합성 호출(`trend-scout`)이 **처음으로 성립했다.** ADR-019 phase가 "호출이 성립하지 않았다"고 적은 자리다.

**n=1이다. 이 숫자로 분포를 말하지 마라.** 특히 인용 23건은 이 범위("국내 물류 자동화")의 검색 결과량에 달린 값이다.

---

**(4) 아직 고치지 않은 것 — `context-hunter`는 같은 병을 앓고 있다.**

`generateGrounded`(JSON 강제 경로)는 **폐기하지 않고 남겼다.** `context-hunter`가 그것을 쓴다.

**그리고 같은 증상이 이미 관측된다.** 사용자 DB의 `context` 아티팩트 6건 중 **5건이 `citations: []`다**(성공은 `2026-07-16` run의 3건뿐). 원인은 스카우트와 동일하다 — JSON 강제 경로이기 때문이다.

**그런데도 지금 고치지 않은 이유는 파급의 크기다.** `context-hunter`는 인용 0건이어도 **그냥 진행한다.** 그래서 run이 죽지 않고, 리포트의 출처 귀속만 **조용히 비어 있었다.** 스카우트는 인용 0건을 치명으로 취급하므로(침묵 게이트) 같은 병이 **거기서만 증상으로 터진 것이다. 스카우트는 카나리아였다.**

**이것이 뜻하는 바를 정확히 적는다: 지금까지 나온 리포트 대부분의 출처는 ADR-013이 세운 "코드가 추출한 인용"이 아니라 LLM 자기보고(`sources`)뿐이다.** ADR-012·ADR-013이 막으려던 상태가 조용히 성립해 있었다.

**다음 phase의 입력으로 남긴다.** 스카우트와 달리 `context-hunter`의 산출물(`MarketContextDraft`)은 훨씬 넓고, 산문 → 구조화 변환의 정보 손실을 측정해야 한다. 스카우트에서 검증된 패턴을 그대로 옮기되, **비용(호출 1회 추가)과 손실을 측정하고 옮겨라.** 측정 없이 옮기지 마라 (ADR-016).
