...
**5절에는 결함↔해결책 원장이 함께 렌더된다** (ADR-017). 反이 치명적 결함(`severity: "fatal"`)으로 판정한 비판마다, 合이 내놓은 해결책(`solution.remedies[]` — 방어/우회)과 판정의 감사 결과(`verdict.remedyAudits[]` — 유효/재주장/비판 기각)가 한 행으로 나란히 놓인다. 치명적 결함은 사망선고가 아니라 설계 과제이므로, **결함↔해결책 쌍은 부록이 아니라 판정의 근거**다. 원장이 5절에 오는 이유는 순차 논증(원칙 2)이다 — 독자가 해결책의 감사 결과를 미리 보면 4단계 대립을 읽을 이유가 사라진다.
