...
## 비판 수용 강제 — 결함↔해결책 원장 (remedies)
전달받은 비판(Criticism)의 criticism.points는 각각 id("c1", "c2", …)를 갖는다.
**severity가 fatal인 항목 각각에 대해 remedies에 항목을 하나씩 만들어라.** 하나라도 빠지면 출력은 검증에 실패해 폐기된다.
- **respondsTo** — 해결 대상 비판의 id를 그대로 옮겨 적는다. 전달받지 않은 id를 지어내지 마라.
- **strategy** — defend(방어) 또는 bypass(우회). 위 "合은 절충이 아니라 피벗 전략이다"의 정의를 그대로 따른다.
- **remedy** — 그 결함을 무엇으로 어떻게 푸는지 구체적으로 쓴다. 비판에 수식어만 덧붙여 같은 말을 되풀이하거나("경량화된 센서", "고도화된 모델"), 우려가 과장됐다며 기각하고 넘어가는 것은 해결책이 아니다.
severity가 major·minor인 항목도 remedies에 넣을 수 있으나 강제하지 않는다.
remedies는 revisedConcept을 대체하지 않는다. 재설계안 본문은 그대로 쓰고, remedies는 그 본문이 어느 결함을 어떻게 짚었는지 항목별로 드러내는 원장이다.
...
3. 위 반론 JSON의 points 중 severity가 fatal인 항목 각각에 대해 remedies 항목을 하나씩 만들어라 — respondsTo에 그 비판의 id를, strategy에 defend(방어)나 bypass(우회)를, remedy에 구체적인 해결책을 쓴다. 하나라도 빠지면 검증에 실패한다. major·minor 항목은 선택이다.
4. 각 결함에 대한 대응은 revisedConcept 본문에도 드러나야 한다. 원장은 본문을 대신하지 않는다.`;
...
export async function runSolutionDesigner(
  ...
  return deps.gemini.generateStructured({
    ...
    // 정적 스키마가 아니라 criticism을 아는 팩토리다 — fatal 침묵은 여기서 검증에 실패하고,
    // 빠진 id를 지목하는 에러가 그대로 자가 교정 재시도의 피드백이 된다 (ADR-017 / ADR-004)
    schema: solutionSchemaFor(criticism),
  });
}
